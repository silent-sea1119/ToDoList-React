...
var SearchBox = React.createClass({
	...
	render: function () {
		return React.createElement(
			...
			React.createElement('input', { className: 'new-todo', type: 'text', placeholder: 'what will I do', value: this.state.text, onChange: this.handlechange, onKeyDown: this.test }),
			React.createElement('input', { className: 'toggle-all', type: 'checkbox', onClick: this.props.toggleAll })
		);
	}
});

var OutcomeList = React.createClass({
	displayName: 'OutcomeList',
		...
		this.props.items.forEach(function (element, index, array) {
			...
			nodes.push(React.createElement(
				...
				React.createElement(
					'div',
					{ className: 'view' },
					React.createElement('input', { className: 'toggle', type: 'checkbox', checked: element.isCompleted, onClick: self.toggle.bind(null, element) }),
					React.createElement(
						'label',
						null,
						element.name
					),
					React.createElement('button', { onClick: self.deleteTodoitem.bind(self, element), className: 'destroy' })
				)
			));
		});
		return React.createElement(
	...
	render: function () {
		...
		var nodes = keyArray.map(function (element) {
			...
			return React.createElement(
				...
				React.createElement(
					'a',
					{ className: selected, onClick: self.updateFilter },
					element
				)
			);
		});
		return React.createElement(
			...
			React.createElement(
				...
				React.createElement(
					...
					React.createElement(
						'a',
						{ className: 'save', onClick: this.props.upload },
						'Save'
					)
				)
			)
...
var CommonContainer = React.createClass({
	...
	render: function () {
		return React.createElement(
			'div',
			{ className: 'todoapp' },
			React.createElement(SearchBox, { addTodoitem: this.addTodoitem, toggleAll: this.toggleAll, addTodoitem: this.addTodoitem }),
			React.createElement(OutcomeList, { items: this.state.filterdata, toggleItem: this.toggleItem, deleteTodoitem: this.deleteTodoitem }),
			React.createElement(Footer, { todocount: this.state.todocount, upload: this.upload, filterWord: this.state.filterWord, updateFilter: this.updateFilter })
		);
	}
});

ReactDOM.render(React.createElement(CommonContainer, { url: 'http://localhost:3000/api/todo' }), document.getElementById('content'));
